  ...
  switch (action.actionType) {
    case appConstants.APP_ADD_SITE:
      if (action.siteDetail.constructor === Immutable.List) {
        ...
        state = state.set('sites', siteUtil.addSite(state.get('sites'), action.siteDetail, action.tag, action.originalSiteDetail))
      }
      if (action.destinationDetail) {
        state = state.set('sites', siteUtil.moveSite(state.get('sites'), action.siteDetail, action.destinationDetail, false, false, true))
      }
      break
    case appConstants.APP_REMOVE_SITE:
      state = state.set('sites', siteUtil.removeSite(state.get('sites'), action.siteDetail, action.tag))
      break
    case appConstants.APP_MOVE_SITE:
      state = state.set('sites', siteUtil.moveSite(state.get('sites'), action.sourceDetail, action.destinationDetail, action.prepend, action.destinationIsParent, false))
      break
